...
return (
    ...
    {username?.length>0?<h2>Welcome {username}</h2>:null}
      ...
      <Switch>
        ...
        <Route path='/Login' render={()=>tk==='"Admin"'?(
        ...
        <Route path='/EmpDetails/:id' render={()=>tk==='"Employee"'||tk==='"Admin"'?(
          <EmpDetails/>
        ):(
          <Redirect to='/Login'/>
        )}/>
        <Route path='/Admindashboard' render={()=>tk==='"Admin"'?(
            <Admindashboard/>
          ):(
            <Redirect to='/Login'/>
          )} />  
          <Route path='/AddEmployee' render={()=>tk==='"Admin"'?(
            <AddEmployee/>
          ):(
            <Redirect to='/Login'/>
          )} />  
        <Route path='/EmployeeDashboard' render={()=>tk==='"Employee"'||tk==='"Admin"'?(
          <EmployeeDashboard/>
        ):(
          <Redirect to='/Login'/>
        )}/>
...
var LogOut=function Logout(){
  return(
    <div className='btn btn-danger float-right' onClick={()=>clearLog()}>Logout</div>
  )
}
function clearLog(){
